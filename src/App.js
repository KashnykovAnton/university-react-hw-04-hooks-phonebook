...
const App = () => {
  ...
  const checkContactIsExist = (name) => {
    return contacts.find((contact) => toLowerCaseFunction(contact.name) === toLowerCaseFunction(name));
  };
  ...
  const filteredContacts = (value) => {
    return contacts.filter((contact) => toLowerCaseFunction(contact.name).includes(toLowerCaseFunction(value)));
  };
  ...
  return (
    <div className="Container Main">
      ...
      <div className="Container ContactsListSection ">
        ...
        {contacts.length !== 0 && <Filter filterValue={filter} onChange={handleFilterContact} />}
        {contacts.length === 0 && <p>There are no contacts in the phone book!</p>}
